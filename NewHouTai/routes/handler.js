  ...
  //获取专业目录
    ...
    handler(req, res, "productList", {ID:parseInt(req.query.ID)},function(data){
      if(data.length==0){
        ...
        handler(req, res, "directoryList", {CDid:parseInt(req.query.ID)},function(da){
            var obj = {
              data:{
                success:"获取成功",
                courselist:data[0],
    ...
    * */
    ...
    handler(req, res, "directoryList",{_id:new ObjectID(req.query._id)} ,function(data){
        //查询该目录对应的课程数据条
        handler(req, res, "productList",{_id:new ObjectID(req.query._id)},function(result){
          ...
          delProDir(req, res,set[0]._id,function(){
            ...
            handler(req, res, "productList",{_id:new ObjectID(req.body._id)},function(data){
              if(data.result.n==0){
                ...
                req.query.action = 'find';
                //查询操作ID大于当前ID的数据集
                handler(req, res, "productList",{ID:parseInt(req.body.ID)},function(da){
                  //迭代处理数据集中的每条数据的ID
    ...
    handler(req, res, "directoryList",{_id:new ObjectID(req.query._id)} ,function(data){
      ...
      if(data.length==0){
        ...
        handler(req, res, "productList",{_id:new ObjectID(req.query.CDid)},function(result){
          ...
          var strstr = req.query._id+req.query.CDid;
          console.log("****************"+strstr);
